...
"""
  ...
  f90source.write("character(%s), parameter :: branch = '%s' !< branch name when compilation occured\n" % (len(branch), branch))
  if (isModifs):
...
gdb = False
profiling = False
force = True # To force the compilation of every module
...
# we commented the following line. Still needs fixing.
...
if (isNautilus):
  compile_source(filename="mnautilus.f90", extra=["opkda1.f90", "opkda2.f90", "opkdmain.f90"])

if (isOutputs):
  compile_source(filename="mnautilus_outputs.f90")

if (isOutputs_select):
    compile_source(filename="mnautilus_select_outputs.f90")

if (isRates):
  compile_source(filename="mnautilus_rates.f90")

if (isMajor):
  compile_source(filename="mnautilus_major_reactions.f90")
  
if (isTrace):
  compile_source(filename="mnautilus_trace_major.f90")
